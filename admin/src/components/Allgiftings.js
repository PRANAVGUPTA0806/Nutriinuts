...
const Allgiftings = () => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/gifting/`
        );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/gifting/${id}`,
        {
          method: "DELETE",
          headers: {
...
const UpdateProductForm = ({ editingProduct, setShowForm, setProducts, setEditingProduct }) => {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/gifting/${id}`, {
        method: 'PUT',
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
